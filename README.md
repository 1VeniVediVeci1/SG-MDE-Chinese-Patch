...
- 标题菜单保留原版底色、高亮、布局和装饰，仅汉化文字；保留存读档空位的原版 `NO DATA` 图。
- 优化小型 UI 与正文的字重、描边和可读性。
- EasyPatcher 一次应用全部文本、字体、UI 和帮助页更改，无需分别选择。
...
- 程序显示版本统一为 `v1.2`。
- 恢复单一“应用补丁”入口，一次应用全部更改。
- 将“真由喜”“真由嘻”统一纠正为“真由氏”，共修正 251 处。
- 保留原版标题视觉样式及存读档背景，完善中文标签与帮助页。
- 手机标题、列表与提示采用常规字重；优化邮件、TIPS、回看正文的描边表现。
